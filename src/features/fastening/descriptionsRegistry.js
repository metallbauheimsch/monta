...
import { isHvGarnitur, HV_STANDARD_NAME } from "./fasteningRules";

const REGISTRY_KEY = "monta_descriptions_v04";

/** Alte HV-Varianten nicht mehr in der Vorschlagsliste anbieten. */
function isDeprecatedHvSuggestion(name) {
  const s = String(name || "").trim();
  if (!s) return false;
  if (s === HV_STANDARD_NAME) return false;
  return isHvGarnitur(s);
}
...
export function getDescriptionOptions() {
  ...
  const extra = learned.filter(
    (d) => !known.has(String(d).toLowerCase()) && !isDeprecatedHvSuggestion(d)
  );
...
export function rememberDescriptionIfNew(bezeichnung) {
  ...
  if (!name) return;
  if (isDeprecatedHvSuggestion(name)) return;
